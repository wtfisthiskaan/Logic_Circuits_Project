...
string get_partof_string(string str,int part){
  ...
  string name;
	...
	j=0;
	while(str[i]!=' ' && str[i]!='\0' && i<str.length()-1){
		buffer[j]=str[i];
		i++;
		j++;
	}
...
string get_partof_decoder(string str,int part){
  char buffer[15];
  int i=8,j;
  for(j=0;j<15;j++)
		buffer[j]='\0';
...
int main() {
	...
	Gate** circuit= new Gate*[totalgates];
	
  //cout<<totalgates<<othergatesnum<<inpnum<<outpnum;
	//second file reading part, data is going to be written 
  
	ifstream inFile2;
	//inFile2.open("circuit.txt");
  inFile2.open ("circuit.txt", ios::out | ios::app | ios::binary); 
 ...
 /* cout<<inputline<<endl;
  ...
  cout<<line<<endl;  */
	...
	for(i=0;i<inpnum;i++){
		circuit[i]= new input;
		circuit[i][0].set_name(get_partof_string(inputline,i));
    //cout<<circuit[i][0].get_name()<<endl;
		count++;
	}
    ...
    break;
      ...
      int countbow=0;
        ...
        countbow++;
			...
			circuit[count][0].set_input_gate(circuit[lasti],circuit[lastj]);
